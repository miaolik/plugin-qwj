...
@handler(r'^(登录群文件|登录|刷新|刷新二维码)$', name='登录群文件', desc='聊天内出二维码扫码登录并自动提取CK', priority=100, ignore_at_check=True)
async def cmd_login(event, match):
    if not await _require_admin(event):
        return
    await _qr_login_flow(event)

@handler(r'^(登录网页|网页登录)$', name='登录网页', desc='获取浏览器扫码登录链接（图片扫不了时用）', priority=100, ignore_at_check=True)
...
@handler(r'^设置登录地址\s*(\S+)$', name='设置登录地址', desc='配置登录页外网地址', priority=100, block=True, ignore_at_check=True)
...
@handler(r'^群文件登录\s*(.+)', name='群文件登录', desc='保存当前管理员的群文件Cookie', priority=100, block=True, ignore_at_check=True)
...
@handler(r'^(添加管理员|新增管理员|管理员添加|管理员新增)\s*(.*)$', name='添加管理员', desc='把用户(可艾特)加入群文件管理员白名单', priority=100, block=True, ignore_at_check=True)
...
@handler(r'^(?:删除管理员|管理员删除|移除管理员|管理员移除)\s*(.*)$', name='删除管理员', desc='把用户(可艾特)移出群文件管理员白名单', priority=100, block=True, ignore_at_check=True)
...
@handler(r'^(管理员列表|群文件管理员列表)$', name='管理员列表', desc='查看群文件管理员白名单', priority=100, block=True, ignore_at_check=True)
